...
public class HeaderExchangeServer implements ExchangeServer {
	...
	/**
	 * 有没有完成的请求就是在运行状态
	 * 没有完成的请求是在DefaultFuture中
	 * @return
	 */
	private boolean isRunning() {
		Collection<Channel> channels = getChannels();
		for (Channel channel : channels) {
			if (DefaultFuture.hasFuture(channel)) {//有正在运行的请求
				return true;
			}
		}
		return false;
	...
	public void close(final int timeout) {
		if (timeout > 0) {
			...
			if (getUrl().getParameter(Constants.CHANNEL_SEND_READONLYEVENT_KEY,
					false)) {
	...
	public Collection<ExchangeChannel> getExchangeChannels() {
		...
		if (channels != null && channels.size() > 0) {
			for (Channel channel : channels) {
				exchangeChannels.add(HeaderExchangeChannel
	...
	public void reset(URL url) {
		...
		try {
			...
			if (url.hasParameter(Constants.HEARTBEAT_KEY)
				...
				int t = url
						.getParameter(Constants.HEARTBEAT_TIMEOUT_KEY, h * 3);
				if (t < h * 2) {
					throw new IllegalStateException(
							"heartbeatTimeout < heartbeatInterval * 2");
				}
	...
	private void startHeatbeatTimer() {
		stopHeartbeatTimer();// 保证只有一个心跳线程
		if (heartbeat > 0) {
			heatbeatTimer = scheduled.scheduleWithFixedDelay(
					new HeartBeatTask(new HeartBeatTask.ChannelProvider() {
						public Collection<Channel> getChannels() {
							return Collections
									.unmodifiableCollection(HeaderExchangeServer.this
											.getChannels());
						}
